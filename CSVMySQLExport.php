...
<div class="container">
<?php
$filName = "telmember.csv";
$objWrite = fopen("telmember.csv", "w");

$objConnect = mysql_connect("localhost","root","097959710") or die("Error Connect to Database");
$objDB = mysql_select_db("system");
mysql_query("SET NAMES TIS620");
$strSQL = "SELECT * FROM telmember";
$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");

while($objResult = mysql_fetch_array($objQuery))
{
	fwrite($objWrite, "\"$objResult[Global]\",\"$objResult[Mobile]\",\"$objResult[Target]\",");
	fwrite($objWrite, "\"$objResult[SMS]\",\"$objResult[Status]\" \n");
}
fclose($objWrite);
...
echo "<meta http-equiv ='refresh'content='10;URL=index_admin.php'>";
?>
</table>
</body>
</html>